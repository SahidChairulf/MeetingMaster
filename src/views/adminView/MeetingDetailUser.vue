<template>
    <div class="grid lg:grid lg:grid-cols-2 lg:gap-2 grid-cols-1 gap-1 w-auto">
        <div class="px-5 mt-6">
            ...
            <!-- End -->
            <div class="grid grid-cols-2 gap-2 mt-5 mx-auto" v-if="dataUser && dataMeeting">
                <!-- Users card -->
                <div class="flex items-center shadow justify-between px-8 h-32 bg-orange-500 rounded-lg dark:bg-darker">
                    <div>
                        ...
                        <span class="text-3xl font-semibold text-slate-600">{{ dataUser.count }}</span>
                    ...
                    <div>
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                class="w-20 h-20 text-slate-100 dark:text-primary-dark">
                                <path
        ...
        <div class="px-4 mt-6">
            ...
            <div class="grid grid-cols-2 gap-2 mt-5 mx-auto">
                ...
                <div class="flex items-center shadow justify-between px-8 h-32 bg-blue-500 rounded-lg dark:bg-darker" v-if="MeetingsCompleted">
                    <div>
                        <h6
                            class="text-lg font-medium leading-none tracking-wider text-white uppercase dark:text-primary-light">
                            Meeting Completed
                        </h6>
                ...
                <!-- Users card -->
                <div class="flex items-center shadow justify-between px-8 h-32 bg-orange-500 rounded-lg dark:bg-darker" v-if="meetingHigh">
                    <div>
                        <h6
                            class="text-lg font-medium leading-none tracking-wider text-slate-300 uppercase dark:text-primary-light">
                            Meeting Category
                        </h6>
                        <span class="text-3xl font-semibold text-slate-100">{{ meetingHigh }} <span class="text-xs">HIGH</span></span>
                        <span class="inline-block px-2 py-px ml-2 text-xs text-green-500 bg-green-100 rounded-md">
                            +2.6%
                        </span>
                    </div>
                ...
                <!-- Users card -->
                <div class="flex items-center shadow justify-between px-8 h-32 bg-yellow-400 rounded-lg dark:bg-darker" v-if="meetingMiddle">
                    <div>
                        <h6
                            class="text-lg font-medium leading-none tracking-wider text-white uppercase dark:text-primary-light">
                            Meeting Category
                        </h6>
                        <span class="text-3xl font-semibold text-white">{{ meetingMiddle }} <span class="text-xs">MIDDLE</span></span>
                        <span class="inline-block px-2 py-px ml-2 text-xs text-green-500 bg-green-100 rounded-md">
                            +2.6%
                        </span>
                    </div>
    ...
    <div class="grid lg:flex lg:flex-auto grid-cols-1 gap-1 w-auto px-5">
        <!-- MEETING LIST -->
                                ...
                                <table class="w-full my-0 align-middle text-dark border-neutral-200 ">
                                    <thead class="align-bottom">
                                        <tr class="font-semibold text-[0.95rem] text-purple-400">
                                            <th class="pb-3 text-start lg:min-w-[155px] min-w-[100px]">TASK</th>
                                    ...
                                    <tbody>
                                        <tr class="border-b border-dashed last:border-b-0" v-for="data in getMeetings"
                                            :key="data.id">
                                            <td class="p-3 pl-0">
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="flex flex-col justify-start">
                                                        <a href="javascript:void(0)"
                                                            class="mb-1 font-semibold transition-colors duration-200 ease-in-out text-lg/normal text-secondary-inverse hover:text-primary truncate w-32">
                                                            {{ data.Meeting.meeting_title }}
                                                        </a>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="p-3 pr-12 text-end">
                                                <span
                                                    ...
                                                    {{ data.Meeting.meeting_status }}
                                                </span>
                                            </td>
                                            <td class="pr-0 text-start">
                                                <span
                                                    class="font-semibold text-light-inverse text-md/normal truncate">{{
                                                        data.Meeting.meeting_schedule }}</span>
                                            </td>
                                            <td class="p-3 pr-0 text-end">
                                                <button @click="showSelectedMeeting(data.Meeting.id)"
            ...
            <div
                ...
                <div class="rounded-lg border bg-white px-4 pt-8 pb-10 shadow-lg">
                    <div class="relative mx-auto w-96">
                        <img class="mx-auto h-auto w-full rounded-full" src="@/assets/img/DashboardUser/CARD-img-1.png"
                            alt="" />
                    </div>
                    ...
                    <div class="grid lg:grid-cols-2 lg:gap-2">
                        <div class="flex items-center py-3 text-sm mt-3 bg-gray-100 px-3 rounded-lg">
                            ...
                            <span class="ml-auto"><span
                                    class="rounded-full py-1 px-2 text-xs font-medium text-slate-900">{{
                                        meeting_title }}</span></span>
                        </div>
                        <div class="flex items-center py-3 text-sm mt-3 bg-gray-100 px-3 rounded-lg">
                            <span>Deskripsi :</span>
                            <span class="ml-auto"><span
                                    class="rounded-full py-1 px-2 text-xs font-medium text-slate-900">{{
                                        meeting_deskripsi }}</span></span>
                        </div>
                        <div class="flex items-center py-3 text-sm mt-3 bg-gray-100 px-3 rounded-lg">
                            <span>Status :</span>
                            <span class="ml-auto"><span
                                    class="rounded-full py-1 px-2 text-xs font-medium text-slate-900">{{
                                        meeting_status }}</span></span>
                        </div>
                        <div class="flex items-center py-3 text-sm mt-3 bg-gray-100 px-3 rounded-lg">
                            <span>Category :</span>
                            <span class="ml-auto"><span
                                    class="rounded-full py-1 px-2 text-xs font-medium text-slate-900">{{
                                        meeting_kategori }}</span></span>
                        </div>
                        <div class="flex items-center py-3 text-sm mt-3 bg-gray-100 px-3 rounded-lg">
                            <span>Address :</span>
                            <span class="ml-auto"><span
                                    class="rounded-full py-1 px-2 text-xs font-medium text-slate-900">{{
                                        meeting_address }}</span></span>
                        </div>
                        <div class="flex items-center py-3 text-sm mt-3 bg-gray-100 px-3 rounded-lg">
                            <span>Date :</span>
                            <span class="ml-auto"><span
                    ...
                    <div class="flex items-center py-3 text-sm bg-gray-100 px-3 rounded-lg mt-3">
                        <span>Evaluasi :</span>
                        <span class="ml-auto"><span
                                class="rounded-full py-1 px-2 text-xs font-medium text-slate-900">{{ meeting_evaluation }}</span></span>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        ...mapGetters('user', ['getUserProfile', 'getAllUser']),
        user() {
        ...
        meetingMiddle() {
            // Filter meeting berdasarkan status 'terjadwal'
            let meetingHigh = this.getMeetings.filter(getMeetings => getMeetings.Meeting.meeting_kategori === 'Middle');
    ...
    methods: {
        ...mapActions('meeting', ['fetchAllMeetings']),
        ...mapActions('user', ['fetchUserProfile', 'fetchAllUser']),
        async showSelectedMeeting(meetingId) {
            try {
                // Dapatkan data pertemuan berdasarkan ID
                ...
                // Perbarui properti data dengan informasi pertemuan yang dipilih
                ...
                // Ubah tanggal pertemuan menjadi objek Date dan format lokal
                const meetingDate = new Date(selectedMeeting.Meeting.meeting_schedule);
                this.date = meetingDate.toLocaleDateString('id-ID', { timeZone: 'UTC' });
